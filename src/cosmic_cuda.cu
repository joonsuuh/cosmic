...
int main(int argc, char* argv[]) {
    ...
    outConfig.setDescriptiveFilename(bhConfig, imgConfig, "cosmic_cuda");
    ...
    std::cout << "Buffer Size: " << numPixels << " pixels" << "\n"
              << "Buffer Size: " << numPixels * sizeof(float) / (1024.0f * 1024.0f) << " MB" << std::endl;
    ...
    CUDA_CHECK(cudaMemcpy(d_noiseMap, noiseMap, noiseSize * noiseSize * sizeof(float), cudaMemcpyHostToDevice));
    timer.stop();
    ...
    CUDA_CHECK(cudaMemcpy(h_screenBuffer, d_screenBuffer, numPixels * sizeof(float), cudaMemcpyDeviceToHost));
    timer.stop();
    ...
    writeImageToFile(outConfig.getFullPath(), h_screenBuffer, pixelWidth, pixelHeight);
    timer.stop();
...
void applyHotColorMap(float value, int& r, int& g, int& b) {
    ...
    if (value < RED_THRESHOLD) {
        ...
        r = 255;
        g = 255;
        b = static_cast<int>(((value - YELLOW_THRESHOLD) / (1.0f - YELLOW_THRESHOLD)) * 255);
    }
}

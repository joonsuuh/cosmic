...
int main() {
    ...
    // Create window
    GLFWwindow* window = glfwCreateWindow(800, 450, "Black Hole Raytracer", NULL, NULL);
    if (!window) {
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < resx; i++) {
            for (int j = 0; j < resy; j++) {
                ...
                // Calculate observed intensity with gravitational redshift and doppler boosting
                double Iobs = 0.0;
    ...
    // Upload texture data
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, resx, resy, 0, GL_RED, GL_FLOAT, textureData.data());
